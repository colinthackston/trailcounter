...
//function for setting the GPIO pins on board
void set_state(uint8_t state){
  int i; 
  for(i = 0; i<5;i++){
  ((state>>i) & 1) ? palSetPad(GPIOD, i+11) : palClearPad(GPIOD, i+11)  ; 
  }
}
...
int main(void) {
  ...
  // palSetPad(GPIOD, 14, PAL_MODE_INPUT_ANALOG);//seeting pins for reading power mode. 
  //palSetPad(GPIOD, 15, PAL_MODE_INPUT_ANALOG);
  /*
